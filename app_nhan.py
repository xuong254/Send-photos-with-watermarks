...
def receive_file():
    ...
    if not file:
        ...
        return jsonify({"status": "fail", "message": "Không có file"}), 400
    ...
    try:
        ...
        print(f"✅ [MÁY NHẬN] Đã nhận file: {filename} từ {sender_ip}")

        if callback_url:
            try:
                print(f"↪️ Đang gửi ACK tới: {callback_url}")
                response = requests.post(callback_url, data={
                    "status": "ACK",
                    "message": f"Đã nhận file: {filename}"
                }, timeout=5)

                if response.status_code == 200:
                    print(f"✅ Gửi phản hồi thành công (status {response.status_code})")
                else:
                    print(f"[⚠️] Phản hồi trả về lỗi {response.status_code}: {response.text}")

            except requests.exceptions.RequestException as e:
                print(f"[❌] Gửi phản hồi thất bại: {e}")
